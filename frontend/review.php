...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO reviews (ProductID, email, rating, comment, created_at) 
            VALUES ('$product_id', '$email', '$rating', '$comment', NOW())";
    ...
    <div class="reviews">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<p class='comment'>" . nl2br(htmlspecialchars($row["comment"])) . "</p></div>";
            }
        } else {
            echo "No reviews yet.";
        }
